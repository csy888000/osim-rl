...
def excel_table_byindex_raw(startRow, file='normalWalkingTest2.xls', colnameindex=8, by_index=0):
    data = open_excel(file)
    table = data.sheets()[by_index]
    nrows = table.nrows #行数
    ncols = table.ncols #列数
    ...
    # nrows = 11
    dataList =[]
    dataArray = zeros((int((nrows - startRow)/1), 18))

    # for rowNum in range(0, nrows-startRow):
    rowNum = 0
    while rowNum*1 < nrows-startRow:
        row = table.row_values(rowNum*1+startRow)
        dataArray[rowNum] = row
        # dataArray[rowNum][1:9] = [actNormal[j]/800 for j in range(len(actNormal))]
        # dataArray[rowNum][0] = row[1]
        if row:
            app = {}
